...
public class GameController : MonoBehaviour {
	...
	void GiveRewards() {
		if (Levels[currentLevel - 1].RewardLevels[currentLevel - 1].RewardInfo.Length > 0) {
			for (int i = 0; i < Levels[currentLevel - 1].RewardLevels[currentLevel - 1].RewardInfo.Length; i++) {

				//Give reward here
				if (Levels[currentLevel - 1].RewardLevels[currentLevel - 1].RewardInfo[i].RewardType == RewardTypes.Coins) {
					Debug.Log("Reward # " + i + "-> " + (Rewardamount = CalculateRewardAmount(i)) + " " + RewardTypes.Coins.ToString ());
				}else if (Levels[currentLevel - 1].RewardLevels[currentLevel - 1].RewardInfo[i].RewardType == RewardTypes.Other) {
					Debug.Log("Reward # " + i + "-> " + (Rewardamount = CalculateRewardAmount(i)) + " " + RewardTypes.Other.ToString ());
				}
			}
		}
		else {
	...
	int reward;
	float TimePerct;
	int CalculateRewardAmount(int index){
		...
		for (int i = 0; i < Levels [currentLevel - 1].RewardLevels.Length; i++) {
			if ((int)TimePerct >= Levels [currentLevel - 1].RewardLevels [i].MinTime && (int)TimePerct <= Levels [currentLevel - 1].RewardLevels [i].MaxTime) {
				reward = Levels [currentLevel - 1].RewardLevels [i].RewardInfo[index].RewardAmount;
			}
		}
		return reward;
	}
	...
	void ShowAds(int id) {
		//AdsManager.ShowAdvertisement (id, "Game Play - Level : "+ currentLevel);
	}
